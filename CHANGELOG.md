...
- `BroadSupportEmailEnvironment` и `BroadSupportEmailEnvironment.current()` —
  версия, build, bundle ID, версия системы, модель устройства, локаль и часовой
  пояс читаются у системы, а не перечисляются приложением.
- Короткий `BroadSupportEmailConfiguration.init(..., environment:, ...)`: хосту
  остаются только поля, которые действительно его — recipient, тема, имя
  приложения, версия в App Store, Adapty profile ID, backend user ID, статус
  подписки и вложение.

### Почему

Семь из восемнадцати полей конфигурации не были продуктовым решением, но их
требовали от приложения. Модель устройства каждый проект доставал одним и тем же
блоком рефлексии `Mirror` над `utsname` — скопированным из проекта в проект и
каждый раз написанным чуть иначе. На симуляторе этот блок отвечал про хостовый
Mac, и письма из debug-сборок приходили с «arm64».

Полный init со всеми полями не тронут.
